...
REFORGES = [
    ...
    "Forceful ", "Hurtful ", "Keen ", "Strong ", "Superior ", "Unpleasant ", "Zealous ", "Hyper ", # hyper was missing from the old ones
    # <-- old ones | new ones -->
...
def main():
    ...
    if len(results): # if there's results to print
        ...
        for result in results:
            ...
            print(str(result[0][0]) + " | [cyan]Name: " + str(result[0][1]) + " | [yellow]Item price: {:,}".format(result[0][2]), " | [red]Second lowest BIN: {:,}".format(result[1]), " | [green]Profit: {:,}".format(profit) + "[/green]")
            profit = 0
